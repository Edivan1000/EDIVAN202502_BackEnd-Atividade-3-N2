...
public class CategoriaCertificadoController {
    ...
    public ResponseEntity<CategoriaCertificado> consultar(@PathVariable Long id){
        ...
        CategoriaCertificado cat = this.cRep.findById(id).orElseThrow(()->
    ...
    public ResponseEntity<CategoriaCertificado> alterar(@PathVariable Long id, @RequestBody CategoriaCertificado categoria){

        CategoriaCertificado catLoc = this.cRep.findById(id).orElseThrow(()->
            new ResourceNotFoundException("Categoria não encontrada: " + id));
